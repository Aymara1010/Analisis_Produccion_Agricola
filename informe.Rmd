...
\textbf{2.-} Se determinó que los sectores con mayor producción son aquellos ubicados al sur de la India, siendo esos los estados más montañosos. \

\textbf{Recomendación} 
\

\textbf{3.-} Y por último se observó que  los cultivos con mayor producción (papa, banana, cebolla) son los cultivos que se producen más en poco espacio, por lo que ayuda no solo a su producción sino a su economía 
.\
...
Agricultura_Filtrado <- data.frame(
  Año = años,
  Serie_Verde = c(1.0, 1.3, 1.6, 1.9, 1.8, 2.1, 2.2, 1.9, 1.9, 1.8, 1.7, 1.8) * 1000,
  Serie_Oscura = c(0.9, 1.1, 1.4, 1.6, 1.5, 1.8, 1.8, 1.6, 1.6, 1.5, 1.4, 1.6) * 1000,
  Serie_Amarilla = c(0.2, 0.3, 0.35, 0.4, 0.38, 0.42, 0.43, 0.38, 0.4, 0.35, 0.34, 0.36) * 1000
)


datos_long <- Agricultura_Filtrado %>%
  pivot_longer(cols = starts_with("Serie"), names_to = "Metrica", values_to = "Valor")
...
ggplot(datos_long, aes(x = Año, y = Valor, color = Metrica)) +
  ...
  scale_color_manual(values = c("Serie_Verde" = "#00ffaa", 
                                "Serie_Oscura" = "#0a3d34", 
                                "Serie_Amarilla" = "#d4ff00")) +
